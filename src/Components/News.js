...
const News=(props)=> {
    ...
    const[articles,setarticles]=useState(article.slice(0,5))
    ...
    const handleInfiniteScroll= async()=>{
        try {
            ...
            window.innerHeight+document.documentElement.scrollTop+1>=document.documentElement.scrollHeight){ 
                setpage(page+5) 
                setloading(false)
                setarticles(articles.concat(article.slice(page-5,page)))
                props.setProgress(props.progress+(500/article.length))
                if(article.length<=page+2){
                    setloading(true)
                }
                ...
                <div className="row">
                    ...
                    {articles.map((element) => {
                        ...
                        return <div className="col-md-4" key={element.url}>
                            <NewsItem title={element.name ? element.name : ""} description={element.description ? element.description : ""} imageUrl={element.url} newsUrl={element.url} />
                        </div>
